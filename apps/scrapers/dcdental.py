...
from apps.scrapers.headers.dcdental import LOGIN_PAGE_POST_HEADERS
...
class DCDentalScraper(Scraper):
    ...
    BASE_URL = "https://www.dcdental.com"
    ...
    def login_proc(self):
        ...
        resp = self.session.post(
            f"{self.BASE_URL}/dc-dental/services/Account.Login.Service.ss?n=3&c=1075085",
            headers=LOGIN_PAGE_POST_HEADERS,
            json=json_data,
        )
    ...
    async def get_orders(
        ...
        tasks = []
        while True:
            params = {"c": 1075085, "n": 3, "sort": "trandate,internalid", "order": 1, "page": page}
            if from_date and to_date:
                params["from"] = from_date
                params["to"] = to_date

            url = f"{self.BASE_URL}/dc-dental/services/OrderHistory.Service.ss?{urlencode(params)}"
            order_history_resp = self.session.get(url, headers=headers)
            logger.info(f"Order History: {order_history_resp.status_code}")

            if "records" not in order_history_resp.json():
                break
